...
function simplifyMessage(paramsArray){
    ...
    //   createdGeometries.push(geometryRaw)
    ...
    //   createdGeometries.push(globalGeometry)
    ...
    //   After ${newGeo.vertices.length}`;
      ...
      let fromGeometry = new ONE.BufferGeometry().fromGeometry(newGeo)
    //   createdGeometries.push(fromGeometry)
      

      return isBufferGeometry ? fromGeometry : newGeo;
    }
...
return meshes
    ...
    function simplifyMesh(geometryRaw, percentage, preserveTexture) {
      ...
      //   After ${newGeo.vertices.length}`;
        ...
        // console.log(`face change from ${geometry.faces.length} to ${newGeo.faces.length}`);
        // createdGeometries.push(newGeo)
        let bufferFrom = new ONE.BufferGeometry().fromGeometry(newGeo)
        // createdGeometries.push(bufferFrom)

        return isBufferGeometry ? bufferFrom : newGeo;
      }
    ...
    function mergeBufferAttributes( attributes ) {
        ...
        let bufferAttribute = new THREE.BufferAttribute(array, itemSize, normalized)
        // createdGeometries.push(bufferAttribute)
    ...
    function mergeBufferGeometries( geometries, useGroups = false ) {
        ...
        // createdGeometries.push(mergedGeometry)
...
// createdGeometries.push(meshMaterial)
...
//    createdGeometries.push(dotMaterial)
...
// console.log("DOTCOORDS", x,y,z, nonIndexed, randIndex, i, randoms)
...
disposeHierarchy (mesh, disposeNode);
// mesh.geometry.dispose()
    // const newLocal = mesh = 0;
    mesh = undefined;

        // newmesh.add(newintersect)
        // createdGeometries.push(newunion)
        ...
        if(e.data[1].geometry0data.dotsSettings.nodots){
            ...
            if(e.data[1].flowerNum == 2){
                ...
                postMessage(
                    ...
                    [
                    {dot1Flower:simplified[0],
                    dot2Flower:simplified[1],
...
if(e.data[1].flowerNum == 2){
    ...
    //   createdGeometries.push(parsedGeom)
    //   mesh.position.set(e.data[1].geometry0data.position)
...
postMessage(
    ...
    //     matrixWorld:dots.children[0].matrixWorld.elements,
    //     name:dots.children[0].name,
    //     position:dots.children[0].position,
    //     up:dots.children[0].up,
    // },
    dot2Flower:simplified[1],
    // dot2Dots:{
    ...
    //     matrixWorld:dots1.children[0].matrixWorld.elements,
    //     name:dots1.children[0].name,
    //     position:dots1.children[0].position,
    //     up:dots1.children[0].up,
    // }, 
    ...
    for (var i=0; i< createdGeometries.length; i++){
        if(createdGeometries[i].isMesh){
            createdGeometries[i].geometry.dispose()
            createdGeometries[i].material.dispose()
        }
...
// }
    ...
    // console.log(parsedGeom)
    let normalsBA = new THREE.BufferAttribute(normals, 3)
    // createdGeometries.push(normalsBA)
    parsedGeom.attributes.normal= normalsBA
    parsedGeom.attributes.normal.needsUpdate = true
    // console.log(parsedGeom)
    
    let positionsFloat = new Float32Array(positions)
    // createdGeometries.push(positionsFloat)
    let positionsFloatBA = new THREE.BufferAttribute(positionsFloat, 3)
    // createdGeometries.push(positionsFloatBA)
    parsedGeom.attributes.position= positionsFloatBA
    parsedGeom.attributes.position.needsUpdate = true
    ...
    let uvsBA = new THREE.BufferAttribute(uvs, 2)
    // createdGeometries.push(uvsBA)
    parsedGeom.attributes.uv= uvsBA
    parsedGeom.attributes.uv.needsUpdate = true
    let indicesBA = new THREE.BufferAttribute(indices, 1)
    // createdGeometries.push(indicesBA)
    parsedGeom.index = indicesBA
    parsedGeom.index.needsUpdate = true
    parsedGeom.needsUpdate = true;
